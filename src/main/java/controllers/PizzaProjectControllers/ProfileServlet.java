...
public class ProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (password.equals("")) { //checks to see if the user updated their password
            User updatedUser = new User(DaoFactory.getUsersDao().findByEmail((String) request.getSession().getAttribute("email")).getId(), firstName, lastName, email, DaoFactory.getUsersDao().findByEmail((String) request.getSession().getAttribute("email")).getPassword(), address);
            DaoFactory.getUsersDao().update(updatedUser);
            request.getSession().setAttribute("email", email);
            response.sendRedirect("profile");
        } else {
            User updatedUser = new User(DaoFactory.getUsersDao().findByEmail((String) request.getSession().getAttribute("email")).getId(), firstName, lastName, email, Password.hash(password), address);
            DaoFactory.getUsersDao().update(updatedUser);
            response.sendRedirect("profile");
        }
